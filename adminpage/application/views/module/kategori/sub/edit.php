...
<form id="editform" action="javascript:prosesDefault('kategori/editSub/<?=$edit['kategori_sub_id'].'/proses'?>','editform')" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
      ...
      <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <div class="form-group">
            ...
            <input type="color" class="form-control form-control-color" name="is_color" value="<?=$edit['is_color'];?>">
          </div>
      </div>
        ...
        <div class="form-group">
          ...
          <div class="row">
            <?php foreach ($all_sub as $sm) : 
              $check = $this->auth->rowData('m_kategori_sub_det','kategori_sub_lv2_id='.$sm['kategori_sub_lv2_id'].' AND kategori_sub_id='.$edit['kategori_sub_id']); 
              if(!isset($check['kategori_sub_lv2_id'])) $stceck = 0; else $stceck = $check['kategori_sub_lv2_id'];
            ?>
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-6">
              <label style="margin-bottom: 0px;"><input type="checkbox" name="subrole[]" <?php if($sm['kategori_sub_lv2_id']==$stceck){ echo "checked";} ?> value="<?=$sm['kategori_sub_lv2_id']?>"> <?=$sm['nama_kategori']?></label><br>
            </div>
            <?php endforeach; ?>
          </div>
        </div>
