...
function handleFileSelect(event) {
    ...
    if (file) {
        ...
        if (!allowedTypes.includes(file.type) && fileExt !== '.xlsx' && fileExt !== '.xls') {
        ...
        $(event.target).next('.custom-file-label').html("Pilih file Excel...");
    }
}
...
function resetFileInput() {
    $('#excelFile').val('');
    $('#btnUpload').prop('disabled', true);
    $('#excelFile').next('.custom-file-label').html("Pilih file Excel...");
}
...
function uploadFile() {
    ...
    const uploadUrl = (typeof importUrls !== 'undefined' && importUrls.upload) ? importUrls.upload : '/adminpage/produk_import/upload';
...
function loadPreview(page) {
    ...
    const previewUrl = (typeof importUrls !== 'undefined' && importUrls.get_preview) ? importUrls.get_preview : '/adminpage/produk_import/get_preview';
...
function updatePagination(current, total) {
    ...
    if (total > 1) {
        ...
        html += `<li class="page-item ${current == 1 ? 'disabled' : ''}">
            <a class="page-link" href="#" onclick="loadPreview(${current - 1}); return false;">Previous</a>
        </li>`;
        ...
        for (let i = 1; i <= total; i++) {
            if (i === current || (i >= current - 2 && i <= current + 2) || i === 1 || i === total) {
                html += `<li class="page-item ${i === current ? 'active' : ''}">
                    <a class="page-link" href="#" onclick="loadPreview(${i}); return false;">${i}</a>
                </li>`;
        ...
        html += `<li class="page-item ${current == total ? 'disabled' : ''}">
            <a class="page-link" href="#" onclick="loadPreview(${current + 1}); return false;">Next</a>
        </li>`;
    }
...
function processImport() {
    ...
    const processUrl = (typeof importUrls !== 'undefined' && importUrls.process) ? importUrls.process : '/adminpage/produk_import/process';
...
function showResults(data) {
    ...
    if (data.success) {
        ...
        if (data.failed_count > 0) {
            ...
            const failedUrl = (typeof importUrls !== 'undefined' && importUrls.download_failed_rows) ? importUrls.download_failed_rows : '/adminpage/produk_import/download_failed_rows';
            $('#btnDownloadFailed').attr('href', failedUrl);
        } else {
            $('#warningAlert').hide();
        }
...
function resetUpload() {
    ...
    // Clear session data
    const resetUrl = (typeof importUrls !== 'undefined' && importUrls.reset) ? importUrls.reset : '/adminpage/produk_import/reset';
    $.ajax({
        url: resetUrl,
        type: 'POST'
    });
...
function downloadTemplate() {
    const templateUrl = (typeof importUrls !== 'undefined' && importUrls.download_template) ? importUrls.download_template : '/adminpage/produk_import/download_template';
